use anyhow::Result;
use shared::{
    ColorFrame, ColorGradient, LedLayout, NodeDiagnostic, NodeDiagnosticSeverity, RgbaColor, Vec3,
};

use crate::color_math::sample_gradient_hsv;
use crate::node_runtime::{NodeEvaluationContext, RuntimeNode, TypedNodeEvaluation};

pub(crate) struct CircleSweepNode {
    cached_layout: Option<CachedLayout>,
    cached_aspect: f32,
    radial_basis: Vec<f32>,
    phase: f32,
    last_elapsed_seconds: Option<f64>,
    gradient: ColorGradient,
}

#[derive(Clone, PartialEq)]
struct CachedLayout {
    pixel_count: usize,
    width: Option<usize>,
    height: Option<usize>,
    points_3d: Option<Vec<Vec3>>,
}

impl Default for CircleSweepNode {
    /// Builds the default cached state and palette for the radial sweep effect.
    fn default() -> Self {
        Self {
            cached_layout: None,
            cached_aspect: f32::NAN,
            radial_basis: Vec::new(),
            phase: 0.0,
            last_elapsed_seconds: None,
            gradient: default_gradient(),
        }
    }
}

crate::node_runtime::impl_runtime_parameters!(CircleSweepNode {
    gradient: ColorGradient => |value| crate::node_runtime::non_empty_gradient(value, default_gradient()), default default_gradient(),
    ..Self::default()
});

pub(crate) struct CircleSweepInputs {
    speed: f32,
    scale: f32,
    aspect: f32,
}

crate::node_runtime::impl_runtime_inputs!(CircleSweepInputs {
    speed = 0.25,
    scale = 1.0,
    aspect = 1.0,
});

pub(crate) struct CircleSweepOutputs {
    frame: ColorFrame,
}

crate::node_runtime::impl_runtime_outputs!(CircleSweepOutputs { frame });

impl RuntimeNode for CircleSweepNode {
    type Inputs = CircleSweepInputs;
    type Outputs = CircleSweepOutputs;

    /// Advances the sweep phase and renders a radial gradient wave into the active layout.
    fn evaluate(
        &mut self,
        context: &NodeEvaluationContext,
        inputs: Self::Inputs,
    ) -> Result<TypedNodeEvaluation<Self::Outputs>> {
        let Some(layout) = context.render_layout.clone() else {
            return Ok(TypedNodeEvaluation {
                outputs: CircleSweepOutputs {
                    frame: ColorFrame {
                        layout: LedLayout {
                            id: "circle_sweep:unbound".to_owned(),
                            role: ::shared::LedLayoutRole::RenderTarget,
                            pixel_count: 0,
                            width: None,
                            height: None,
                            points_3d: None,
                        },
                        pixels: Vec::new(),
                    },
                },
                frontend_updates: Vec::new(),
                diagnostics: vec![NodeDiagnostic {
                    severity: NodeDiagnosticSeverity::Warning,
                    code: Some("circle_sweep_missing_render_layout".to_owned()),
                    message: "Circle Sweep has no render layout, so it cannot render a frame yet."
                        .to_owned(),
                }],
            });
        };

        let mut diagnostics = Vec::new();
        let scale = inputs.scale.max(0.01);
        if (scale - inputs.scale).abs() > f32::EPSILON {
            diagnostics.push(NodeDiagnostic {
                severity: NodeDiagnosticSeverity::Warning,
                code: Some("circle_sweep_scale_clamped".to_owned()),
                message: format!(
                    "Scale {} is too small; using {} instead.",
                    inputs.scale, scale
                ),
            });
        }
        let aspect = inputs.aspect.max(0.01);
        if (aspect - inputs.aspect).abs() > f32::EPSILON {
            diagnostics.push(NodeDiagnostic {
                severity: NodeDiagnosticSeverity::Warning,
                code: Some("circle_sweep_aspect_clamped".to_owned()),
                message: format!(
                    "Aspect {} is too small; using {} instead.",
                    inputs.aspect, aspect
                ),
            });
        }

        let phase = self.advance_phase(context.elapsed_seconds, inputs.speed);
        self.ensure_radial_basis(&layout, aspect);
        let uses_spatial_basis = layout
            .points_3d
            .as_ref()
            .is_some_and(|points| points.len() >= layout.pixel_count);

        let mut pixels = Vec::with_capacity(layout.pixel_count);
        for i in 0..layout.pixel_count {
            let basis_position = if uses_spatial_basis {
                self.radial_basis[i] / (scale * 100.0)
            } else {
                scale * self.radial_basis[i]
            };
            let gradient_position = fract01(phase + basis_position);
            pixels.push(sample_gradient_hsv(&self.gradient, gradient_position));
        }

        Ok(TypedNodeEvaluation {
            outputs: CircleSweepOutputs {
                frame: ColorFrame { layout, pixels },
            },
            frontend_updates: Vec::new(),
            diagnostics,
        })
    }
}

impl CircleSweepNode {
    /// Integrates the animated sweep phase from elapsed time and the configured speed.
    fn advance_phase(&mut self, elapsed_seconds: f64, speed: f32) -> f32 {
        let dt = match self.last_elapsed_seconds {
            Some(last_elapsed_seconds) if elapsed_seconds >= last_elapsed_seconds => {
                (elapsed_seconds - last_elapsed_seconds) as f32
            }
            _ => 0.0,
        };
        self.last_elapsed_seconds = Some(elapsed_seconds);
        self.phase = fract01(self.phase + speed * dt);
        self.phase
    }

    /// Recomputes the cached radial basis when the layout shape or aspect ratio changes.
    fn ensure_radial_basis(&mut self, layout: &LedLayout, aspect: f32) {
        let cached_layout = CachedLayout {
            pixel_count: layout.pixel_count,
            width: layout.width,
            height: layout.height,
            points_3d: layout.points_3d.clone(),
        };
        if self.cached_layout.as_ref() == Some(&cached_layout) && self.cached_aspect == aspect {
            return;
        }

        self.radial_basis.clear();
        self.radial_basis.reserve(layout.pixel_count);

        if let Some(points) = layout
            .points_3d
            .as_ref()
            .filter(|points| points.len() >= layout.pixel_count)
        {
            self.radial_basis = spatial_radial_basis(points, layout.pixel_count, aspect);
        } else {
            let (width, height) = layout_dims(layout);
            let center_x = (width.saturating_sub(1)) as f32 * 0.5;
            let center_y = (height.saturating_sub(1)) as f32 * 0.5;
            let max_radius = ((center_x / aspect).powi(2) + center_y.powi(2))
                .sqrt()
                .max(f32::EPSILON);
            for i in 0..layout.pixel_count {
                let (x, y) = pixel_xy(i, layout, width);
                let dx = (x - center_x) / aspect;
                let dy = y - center_y;
                self.radial_basis
                    .push(((dx * dx + dy * dy).sqrt() / max_radius).clamp(0.0, 1.0));
            }
        }
        self.cached_layout = Some(cached_layout);
        self.cached_aspect = aspect;
    }
}

/// Builds the default rainbow-like palette used by the circle sweep.
fn default_gradient() -> ColorGradient {
    ColorGradient {
        stops: vec![
            stop(0.0, 1.0, 0.0, 0.0),
            stop(0.2, 1.0, 0.5, 0.0),
            stop(0.4, 1.0, 1.0, 0.0),
            stop(0.6, 0.0, 1.0, 0.0),
            stop(0.8, 0.0, 0.4, 1.0),
            stop(1.0, 0.7, 0.0, 1.0),
        ],
    }
}

/// Creates one opaque gradient stop for the circle-sweep palette.
fn stop(position: f32, r: f32, g: f32, b: f32) -> shared::ColorGradientStop {
    shared::ColorGradientStop {
        position,
        color: RgbaColor { r, g, b, a: 1.0 },
    }
}

/// Returns the effective raster dimensions used to compute radial distances.
fn layout_dims(layout: &LedLayout) -> (usize, usize) {
    match (layout.width, layout.height) {
        (Some(width), Some(height)) if width > 0 && height > 0 => (width, height),
        _ => (layout.pixel_count.max(1), 1),
    }
}

/// Returns the pixel-space coordinates for one layout index.
fn pixel_xy(index: usize, layout: &LedLayout, width: usize) -> (f32, f32) {
    if let (Some(layout_width), Some(layout_height)) = (layout.width, layout.height) {
        if layout_width > 0 && layout_height > 0 {
            let x = (index % layout_width) as f32;
            let y = (index / layout_width).min(layout_height - 1) as f32;
            return (x, y);
        }
    }

    (index.min(width.saturating_sub(1)) as f32, 0.0)
}

/// Computes raw radial distances from the spatial origin in 3D space.
fn spatial_radial_basis(points: &[Vec3], pixel_count: usize, aspect: f32) -> Vec<f32> {
    let mut radii = Vec::with_capacity(pixel_count);
    for point in points.iter().take(pixel_count) {
        let radius = ((point.x / aspect).powi(2) + point.y.powi(2) + point.z.powi(2)).sqrt();
        radii.push(radius);
    }
    radii
}

/// Wraps a float into the `[0, 1)` interval.
fn fract01(v: f32) -> f32 {
    let f = v.fract();
    if f < 0.0 { f + 1.0 } else { f }
}

#[cfg(test)]
mod tests {
    use shared::{LedLayout, Vec3};

    use super::CircleSweepNode;

    /// Tests that the circle-sweep phase accumulates elapsed time scaled by speed.
    #[test]
    fn phase_integrates_speed_over_time() {
        let mut node = CircleSweepNode::default();

        assert_eq!(node.advance_phase(0.0, 0.25), 0.0);
        assert!((node.advance_phase(1.0, 0.25) - 0.25).abs() < 1e-6);
        assert!((node.advance_phase(2.0, 1.0) - 0.25).abs() < 1e-6);
        assert!((node.advance_phase(3.0, 1.0) - 0.25).abs() < 1e-6);
    }

    /// Tests that spatial layouts preserve absolute world distance from the origin.
    #[test]
    fn spatial_layout_uses_world_distance_radial_basis() {
        let mut node = CircleSweepNode::default();
        let layout = LedLayout {
            id: "spatial".to_owned(),

            role: ::shared::LedLayoutRole::RenderTarget,
            pixel_count: 3,
            width: Some(3),
            height: Some(1),
            points_3d: Some(vec![
                Vec3 {
                    x: 0.0,
                    y: 0.0,
                    z: 0.0,
                },
                Vec3 {
                    x: 0.5,
                    y: 0.0,
                    z: 0.0,
                },
                Vec3 {
                    x: 0.0,
                    y: 0.0,
                    z: 1.0,
                },
            ]),
        };

        node.ensure_radial_basis(&layout, 1.0);

        assert_eq!(node.radial_basis.len(), 3);
        assert!((node.radial_basis[0] - 0.0).abs() < 1e-6);
        assert!((node.radial_basis[1] - 0.5).abs() < 1e-6);
        assert!((node.radial_basis[2] - 1.0).abs() < 1e-6);
    }

    /// Tests that spatial radial distances are not normalized per sink layout.
    #[test]
    fn spatial_layout_does_not_normalize_to_local_max_radius() {
        let mut node = CircleSweepNode::default();
        let layout = LedLayout {
            id: "spatial".to_owned(),

            role: ::shared::LedLayoutRole::RenderTarget,
            pixel_count: 2,
            width: Some(2),
            height: Some(1),
            points_3d: Some(vec![
                Vec3 {
                    x: 2.0,
                    y: 0.0,
                    z: 0.0,
                },
                Vec3 {
                    x: 4.0,
                    y: 0.0,
                    z: 0.0,
                },
            ]),
        };

        node.ensure_radial_basis(&layout, 1.0);

        assert_eq!(node.radial_basis.len(), 2);
        assert!((node.radial_basis[0] - 2.0).abs() < 1e-6);
        assert!((node.radial_basis[1] - 4.0).abs() < 1e-6);
    }
}
