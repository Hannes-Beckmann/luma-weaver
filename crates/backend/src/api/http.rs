...
async fn upload_image_asset(
    ...
    let asset_id = state
        ...
        .map_err(invalid_image_upload_response)?;
    Ok(Json(UploadImageAssetResponse { asset_id }))
}

fn invalid_image_upload_response(error: anyhow::Error) -> (StatusCode, String) {
    let message = error.to_string();
    let status = if message.contains("decode uploaded image")
